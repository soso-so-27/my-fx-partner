...
export default function Home() {
  ...
  const loadData = async () => {
    ...
    if (!session?.user?.email) {
      if (status === 'unauthenticated') setIsInitialLoading(false)
      return
    }
  ...
  useEffect(() => {
    loadData()
  }, [session, status, currentMonth, currentWeek, calendarView])
  ...
  if (isInitialLoading) {
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-background">
        <div className="mb-4 text-4xl font-bold tracking-tighter text-primary animate-pulse">
          SOLO
        </div>
